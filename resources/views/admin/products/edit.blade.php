            ...
            <div class="bcard">
                ...
                <div class="bcard-body">
                                ...
                                @if($cat->children && $cat->children->count() > 0)
                                    <optgroup label="{{ $cat->name }}">
                                        ...
                                        @foreach($cat->children as $child)
                                            <option value="{{ $child->slug }}" {{ old('category', $product->category) == $child->slug ? 'selected' : '' }}>— {{ $child->name }}</option>
                                        @endforeach
                                    </optgroup>
                                @else
                                    <option value="{{ $cat->slug }}" {{ old('category', $product->category) == $cat->slug ? 'selected' : '' }}>{{ $cat->name }}</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="sizes" class="form-control" value="{{ old('sizes', is_array($product->sizes) ? implode(', ', $product->sizes) : $product->sizes) }}" placeholder="e.g. S, M, L, XL or 32, 34, 36">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="label" class="form-select">
                            ...
                            @foreach(['Trending Now', 'Limited Edition', 'Customer Favourite', 'New Arrival'] as $opt)
                                <option value="{{ $opt }}" {{ old('label', $product->label) === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
